...
class WAConnection extends events_1.EventEmitter {
    constructor() {
        ...
        this.browserDescription = Utils.Browsers.amdiweb('Chrome');
        ...
        this.logger = logger.child({ class: 'AmdiWeb' });
        /** log messages */
    ...
    async waitForMessage(tag, requiresPhoneConnection, timeoutMs) {
        ...
        try {
            const result = await Utils.promiseTimeout(timeoutMs, (resolve, reject) => {
                onRecv = resolve;
                onErr = ({ reason, status }) => reject(new Constants_1.AmdiWebError(reason, { status }));
        ...
        while (triesLeft >= 0) {
            ...
            try {
                ...
                if (expect200 && response.status && Math.floor(+response.status / 100) !== 2) {
                    ...
                    throw new Constants_1.AmdiWebError(`Unexpected status in '${json[0] || 'query'}': ${http_1.STATUS_CODES[response.status]}(${response.status})`, { query: json, message, status: response.status });
                }
    ...
    async waitForConnection() {
        ...
        if (this.pendingRequestTimeoutMs !== null && this.pendingRequestTimeoutMs <= 0) {
            throw new Constants_1.AmdiWebError(Constants_1.DisconnectReason.close, { status: 428 });
        }
